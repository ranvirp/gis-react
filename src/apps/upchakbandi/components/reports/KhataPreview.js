...
const khataQuery = 'query a($filter:String!){khata_by_filter(filter:$filter){id  khatauni{village{vname}} khata_no category{category name} area}}'
...
export function KhataPreview (props)
{
    const newForm = useForm()
    const [khata_no,setKhataNo] = useState(0)
    const {items} = useGraphQlQuery(khataQuery, genFilter(localStorage.khatauni_id,khata_no), 'khata_by_filter')
    function fn1(e) {
    ...
    return (<Stack>
        ...
        <KhatedarPreview sx={{width:"100%"}} khata_no={khata_no} khatauni_id={localStorage.khatauni_id}/>

                <GataPreview sx={{width:"100%"}} khata_no={khata_no} khatauni_id={localStorage.khatauni_id}/>
...
export function GataPreview({khata_no, khatauni_id, ...props})
{
    ...
    const {items} = useGraphQlQuery(gataQuery,genFilter(khatauni_id, khata_no),'gata_by_filter')
    console.log(items)
    return (
