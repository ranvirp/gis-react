...
export const createUpdateOrdermodificationMutation = `mutation something($id:String, $court_order:String!, $order:String!){
    create_update_ordermodification(id: $id, court_order: $court_order, order: $order) {
...
function search(){
  ...
  graphqlFetch(query,{filter:JSON.stringify(data)},'ordermodification_by_filter').then(({items,errors,success})=>{
    if (success && items.length==1) {
       form.setValue("court_order", items[0]["court_order"]["id"])
form.setValue("order", items[0]["order"]["id"])
...
Object.keys(data).map(value=>{
    ...
    graphqlFetch(createUpdateOrdermodificationMutation, data, 'create_update_ordermodification').then(({items,errors,success})=>{
 if (errors.length > 0) {
...
<ReactHookFormInput  form={form} comp={defaultComponents["order"]} name={"order"} label={"Order"}/>
