...
export const createUpdateGataOwnerVersionMutation = `mutation something($id:String, $khatauni:String!, $gata:String!, $date_of_ownership:String!, $owner:String!, $owner_details:String!, $owned_share:String!, $owned_area:String!){
    create_update_gata_owner_version(id: $id, khatauni: $khatauni, gata: $gata, date_of_ownership: $date_of_ownership, owner: $owner, owner_details: $owner_details, owned_share: $owned_share, owned_area: $owned_area) {
...
const query = 'query a($filter:String!){gata_owner_version_by_filter(filter:$filter) { id khatauni{ id  village{ vname  village_code_census  vname_eng  pname  flg_chakbandi  flg_survey  pargana_code_new  tehsil_code{ district_code{ dname  district_code_census  dname_eng }  tname  tehsil_code_census  tname_eng }  district_code{ dname  district_code_census  dname_eng } }  fasli_year  no_of_khatas  no_of_gatas  area  status }  gata  date_of_ownership owner{ id  chakbandi{ id  date_of_4notification  village{ vname  village_code_census  vname_eng  pname  flg_chakbandi  flg_survey  pargana_code_new  tehsil_code{ district_code{ dname  district_code_census  dname_eng }  tname  tehsil_code_census  tname_eng }  district_code{ dname  district_code_census  dname_eng } }  status }  khatauni{ id  village{ vname  village_code_census  vname_eng  pname  flg_chakbandi  flg_survey  pargana_code_new  tehsil_code{ district_code{ dname  district_code_census  dname_eng }  tname  tehsil_code_census  tname_eng }  district_code{ dname  district_code_census  dname_eng } }  fasli_year  no_of_khatas  no_of_gatas  area  status }  unique_id  khatedar_code  mobile_no  name  minimized_name  name_eng  minimized_name_eng  relationship  relative_name  relative_name_minimized  year_of_birth  gender  address  status }  owner_details  owned_share  owned_area }  }'
...
function search(){
  ...
  graphqlFetch(query,{filter:JSON.stringify(data)},'gata_owner_version_by_filter').then(({items,errors,success})=>{
...
Object.keys(data).map(value=>{
    ...
    graphqlFetch(createUpdateGataOwnerVersionMutation, data, 'create_update_gata_owner_version').then(({items,errors,success})=>{
 if (errors.length > 0) {
